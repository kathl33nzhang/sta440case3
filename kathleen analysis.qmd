...
# (Assumes to_posix() and ems are already defined in earlier chunk)
...
# ---- start of CHUNK 4 (rebuild inc, then simulate) ----
inc <- ems_for_inc %>%
...
# Explicit list-column; each element is a *named* numeric vector
...
# optional assert to catch issues early
stopifnot(all(vapply(
  scenarios$units_cfg,
  function(x) !is.null(names(x)) && length(names(x)) == length(x),
  logical(1)
)))
...
# Optional quick diag
# str(sim_results$total_response_secs); summary(sim_results$total_response_secs)
...
# Uses sim_results_clean (travel+wait only) created at end of Chunk 4
