...
std_names <- function(df) {
  nm <- names(df)
  nm[nm == "incident_id"] <- "Incident_ID"
  nm[nm == "scenario"]    <- "Scenario"
  names(df) <- nm
  df
}
```
...
simulate_scenario <- function(incidents, units_cfg_named_vec) {
  incidents <- std_names(incidents)
  # require Incident_ID / dispatch_at / service_secs / tt_* cols
  if (!all(c("Incident_ID","dispatch_at","service_secs") %in% names(incidents))) {
    stop("incidents must have Incident_ID, dispatch_at, service_secs")
  }

  # normalize scenario config
  counts_raw <- units_cfg_named_vec
  if (is.list(counts_raw)) counts_raw <- unlist(counts_raw, recursive = TRUE, use.names = TRUE)
  counts <- as.integer(counts_raw); nms <- names(counts_raw)
  if (is.null(nms) || anyNA(nms) || length(nms) == 0) stop("units_cfg must be a *named* numeric vector")
  ...
  for (i in seq_len(nI)) {
    ...
    svc <- incidents$service_secs[i]; if (is.na(svc) || svc < 0) svc <- 0
    ...
    avail <- fleet$next_free <= call_t; avail[is.na(avail)] <- FALSE

    if (!any(avail)) {
      nf <- as.numeric(fleet$next_free); nf[is.na(nf)] <- Inf
      chosen <- which.min(nf); if (!length(chosen)) chosen <- 1
      ...
      if (!any(is.finite(finite_tt))) {
        chosen <- idx[1]; travel_s <- 0
      } else {
        chosen <- idx[which.min(finite_tt)]; travel_s <- tt_vals[chosen]
      }
      wait_s <- 0; depart_time <- call_t
    }

    # IMPORTANT: busy time = wait + service (service is dispatch→available already)
    fleet$next_free[chosen] <- depart_time + lubridate::seconds(wait_s + svc)

    out[[i]] <- tibble::tibble(
      Incident_ID         = incidents$Incident_ID[i],   # standardized name
      assigned_from       = fleet$station[chosen],
      assigned_unit       = fleet$unit_id[chosen],
      wait_secs           = wait_s,
      travel_secs         = travel_s,
      sim_time            = wait_s + travel_s          # dispatch→arrival + queue
    )
  }
...
# Use the standardized incidents frame:
inc <- ems_for_inc %>%
...
sim_results <- scenarios %>%
  mutate(sim = purrr::map(units, ~ simulate_scenario(inc, .x))) %>%
  select(scenario, sim) %>%
  tidyr::unnest(sim) %>%
  ...
  mutate(
    ...
    sim_time            = as.numeric(sim_time),             # response (dispatch→arrival)
    total_time_secs     = sim_time + service_secs           # dispatch→available under scenario
  )

sim_results_clean <- sim_results %>%
  filter(is.finite(sim_time),
         sim_time >= 0,
         sim_time <= 2*60*60)
...
print(sapply(sim_results_clean %>%
               select(scenario, wait_secs, travel_secs, service_secs,
                      sim_time, total_time_secs),
             function(x) paste(class(x), collapse=" / ")))

cat("\n--- quick summaries (should be a few hundred seconds) ---\n")
print(summary(sim_results_clean$sim_time))
...
summ_tbl_check <- sim_results_clean %>%
  group_by(scenario) %>%
  summarise(mean_resp = mean(sim_time), .groups="drop")
print(summ_tbl_check)


```
```{r}
ems_cov <- std_names(ems_simplified) %>%
  distinct(Incident_ID, .keep_all = TRUE)

ems_simplified_new <- sim_results_clean %>%
  left_join(ems_cov, by = "Incident_ID") %>%
  relocate(Incident_ID, scenario, sim_time, .before = dplyr::everything()) %>%
  drop_na()

ems_model <- ems_simplified_new
```

```{r}
# Response-time summaries (travel + wait)
summ_tbl <- ems_simplified_new %>%
  group_by(scenario) %>%
  summarise(
    n               = n(),
    mean_resp_s     = mean(sim_time),
...
dropped_ems_model <- ems_model |> 
  ...
  filter(!all(sim_time == first(sim_time))) |> 
  ungroup()
```
...
complete_ids <- ems_model %>%
  count(Incident_ID, Scenario) %>%
  count(Incident_ID, name = "nScen") %>%
  filter(nScen == n_distinct(ems_model$Scenario)) %>%
  pull(Incident_ID)
...
ems <- ems |>
  group_by(DT.DISP, BASE.NAME) |>
  mutate(
Incident_ID = cur_group_id(), Patient_Count = n(), Patient_Number = row_number() ) |>
  ungroup()
ems_filtered <- ems |>
  filter(Patient_Number == 1)
```
...
```{r}
# Scenario 1: Central (3) and South (1)
central_avail <- 3
south_avail <- 1
# Initialize columns with S1_ prefix
ems_filtered$S1_Ambulances_Central <- NA
ems_filtered$S1_Ambulances_South <- NA
ems_filtered$S1_Assigned_Station <- NA
ems_filtered$S1_Delay <- 0
# Track when each ambulance becomes available
central_return_times <- rep(min(ems_filtered$DT.DISP) - 1, central_avail)
south_return_times <- rep(min(ems_filtered$DT.DISP) - 1, south_avail)
for(i in 1:nrow(ems_filtered)) {
current_time <- ems_filtered$DT.DISP[i]
service_duration <- ems_filtered$DT.AVAILABLE[i] - ems_filtered$DT.DISP[i]
# Count available ambulances
avail_Ce <- sum(central_return_times <= current_time)
avail_So <- sum(south_return_times <= current_time)
# Record current availability
ems_filtered$S1_Ambulances_Central[i] <- avail_Ce
ems_filtered$S1_Ambulances_South[i] <- avail_So
# Check if any ambulances are available
if(avail_Ce == 0 & avail_So == 0) {
# NO AMBULANCES AVAILABLE - wait for next one
next_central <- min(central_return_times)
next_south <- min(south_return_times)
if(next_central <= next_south) {
ems_filtered$S1_Assigned_Station[i] <- "Central"
wait_time <- next_central - current_time
next_ambulance <- which.min(central_return_times)
central_return_times[next_ambulance] <- next_central + service_duration
} else {
ems_filtered$S1_Assigned_Station[i] <- "South"
wait_time <- next_south - current_time
next_ambulance <- which.min(south_return_times)
south_return_times[next_ambulance] <- next_south + service_duration
}
ems_filtered$S1_Delay[i] <- wait_time
} else {# AMBULANCES AVAILABLE - assign based on travel time 
  if(ems_filtered$eTT.Op.Ce[i] <= ems_filtered$eTT.Op.So[i] & avail_Ce > 0) {
ems_filtered$S1_Assigned_Station[i] <- "Central"
next_ambulance <- which(central_return_times <= current_time)[1]
central_return_times[next_ambulance] <- current_time + service_duration
} else if (ems_filtered$eTT.Op.So[i] < ems_filtered$eTT.Op.Ce[i] & avail_So > 0) {
ems_filtered$S1_Assigned_Station[i] <- "South"
next_ambulance <- which(south_return_times <= current_time)[1]
south_return_times[next_ambulance] <- current_time + service_duration
} else if (avail_Ce > 0) {
ems_filtered$S1_Assigned_Station[i] <- "Central"
next_ambulance <- which(central_return_times <= current_time)[1]
central_return_times[next_ambulance] <- current_time + service_duration
...
ems_filtered$S1_Delay[i] <- 0
}
}
# Calculate S1 performance metrics
ems_filtered$S1_Actual_Travel_Time <- ifelse(ems_filtered$S1_Assigned_Station == "Central",
ems_filtered$eTT.Op.Ce,
ems_filtered$eTT.Op.So)
ems_filtered$S1_Total_Response_Time <- ems_filtered$S1_Actual_Travel_Time + 60 * ems_filtered$S1_Delay

```

```{r}
# Scenario 2: Central (3) and Near North (1)
central_avail <- 3
nearnorth_avail <- 1
# Initialize columns with S2_ prefix
ems_filtered$S2_Ambulances_Central <- NA
ems_filtered$S2_Ambulances_NearNorth <- NA
ems_filtered$S2_Assigned_Station <- NA
ems_filtered$S2_Delay <- 0
# Track when each ambulance becomes available
central_return_times <- rep(min(ems_filtered$DT.DISP) - 1, central_avail)nearnorth_return_times <- rep(min(ems_filtered$DT.DISP) - 1, nearnorth_avail)
for
(i
in
1
:nrow(ems_filtered)) { current_time <- ems_filtered$DT.DISP[i] service_duration <- ems_filtered$DT.AVAILABLE[i] - ems_filtered$DT.DISP[i]
# Count available ambulances
avail_Ce <- sum(central_return_times <= current_time) avail_NN <- sum(nearnorth_return_times <= current_time)
# Record current availability
ems_filtered$S2_Ambulances_Central[i] <- avail_Ce 
ems_filtered$S2_Ambulances_NearNorth[i] <- avail_NN
# Check if any ambulances are available
if
(avail_Ce ==
0
...
# NO AMBULANCES AVAILABLE - wait for next one
next_central <- min(central_return_times) 
next_nearnorth <- min(nearnorth_return_times)
if
(next_central <= next_nearnorth) { ems_filtered$S2_Assigned_Station[i] <- "Central"
wait_time <- next_central - current_time 
next_ambulance <- which.min(central_return_times) 
central_return_times[next_ambulance] <- next_central + service_duration }
else
{ ems_filtered$S2_Assigned_Station[i] <-
"NearNorth"
wait_time <- next_nearnorth - current_time 
next_ambulance <- which.min(nearnorth_return_times) 
nearnorth_return_times[next_ambulance] <- next_nearnorth + service_duration } 
ems_filtered$S2_Delay[i] <- wait_time }
else
{
# AMBULANCES AVAILABLE - assign based on travel time
if
(ems_filtered$eTT.Op.Ce[i] <= ems_filtered$eTT.Op.NN[i] & avail_Ce >
0
) { ems_filtered$S2_Assigned_Station[i] <-
"Central"
next_ambulance <- which(central_return_times <= current_time)[
1
] central_return_times[next_ambulance] <- current_time + service_duration }
else
if
(ems_filtered$eTT.Op.NN[i] < ems_filtered$eTT.Op.Ce[i] & avail_NN >
0
) { ems_filtered$S2_Assigned_Station[i] <-
"NearNorth"
next_ambulance <- which(nearnorth_return_times <= current_time)[
1
] nearnorth_return_times[next_ambulance] <- current_time + service_duration }
else
if
(avail_Ce >
0
...
next_ambulance <- which(nearnorth_return_times <= current_time)[
1
] nearnorth_return_times[next_ambulance] <- current_time + service_duration } ems_filtered$S2_Delay[i] <-
0
}}
# Calculate S2 performance metrics
ems_filtered$S2_Actual_Travel_Time <- ifelse(ems_filtered$S2_Assigned_Station ==
"Central"
, ems_filtered$eTT.Op.Ce, ems_filtered$eTT.Op.NN)ems_filtered$S2_Total_Response_Time <- ems_filtered$S2_Actual_Travel_Time +
60* ems_filtered$S2_Delay
```

```{r}
# Scenario 3: Central (3) and Far North (1)
central_avail <-
3
farnorth_avail <-
1
# Initialize columns with S3_ prefix
ems_filtered$S3_Ambulances_Central <-
NA
ems_filtered$S3_Ambulances_FarNorth <-
NA
ems_filtered$S3_Assigned_Station <-
NA
ems_filtered$S3_Delay <-
0
# Track when each ambulance becomes available
central_return_times <- rep(min(ems_filtered$DT.DISP) -
1
, central_avail)farnorth_return_times <- rep(min(ems_filtered$DT.DISP) -
1
, farnorth_avail)
for
(i
...
:nrow(ems_filtered)) { current_time <- ems_filtered$DT.DISP[i] service_duration <- ems_filtered$DT.AVAILABLE[i] - ems_filtered$DT.DISP[i]
# Count available ambulances
avail_Ce <- sum(central_return_times <= current_time) avail_FN <- sum(farnorth_return_times <= current_time)
# Record current availability
ems_filtered$S3_Ambulances_Central[i] <- avail_Ce ems_filtered$S3_Ambulances_FarNorth[i] <- avail_FN
# Check if any ambulances are available
if
(avail_Ce ==
0
...
# NO AMBULANCES AVAILABLE - wait for next one
next_central <- min(central_return_times) next_farnorth <- min(farnorth_return_times)
if
(next_central <= next_farnorth) { ems_filtered$S3_Assigned_Station[i] <-
"Central"
wait_time <- next_central - current_time next_ambulance <- which.min(central_return_times) central_return_times[next_ambulance] <- next_central + service_duration }
else
{ ems_filtered$S3_Assigned_Station[i] <-
"FarNorth"
wait_time <- next_farnorth - current_time next_ambulance <- which.min(farnorth_return_times) farnorth_return_times[next_ambulance] <- next_farnorth + service_duration } ems_filtered$S3_Delay[i] <- wait_time }
else
{
# AMBULANCES AVAILABLE - assign based on travel time
if
(ems_filtered$eTT.Op.Ce[i] <= ems_filtered$eTT.Op.FN[i] & avail_Ce >
0
) { ems_filtered$S3_Assigned_Station[i] <-
"Central"
next_ambulance <- which(central_return_times <= current_time)[
...
if
(ems_filtered$eTT.Op.FN[i] < ems_filtered$eTT.Op.Ce[i] & avail_FN >
0
) { ems_filtered$S3_Assigned_Station[i] <-
"FarNorth"
next_ambulance <- which(farnorth_return_times <= current_time)[
1
] farnorth_return_times[next_ambulance] <- current_time + service_duration }
else
if
(avail_Ce >
0
...
next_ambulance <- which(farnorth_return_times <= current_time)[
1
] farnorth_return_times[next_ambulance] <- current_time + service_duration } ems_filtered$S3_Delay[i] <-
0
}}
# Calculate S3 performance metrics
ems_filtered$S3_Actual_Travel_Time <- ifelse(ems_filtered$S3_Assigned_Station ==
"Central"
, ems_filtered$eTT.Op.Ce, ems_filtered$eTT.Op.FN)ems_filtered$S3_Total_Response_Time <- ems_filtered$S3_Actual_Travel_Time +
60* ems_filtered$S3_Delay
```
